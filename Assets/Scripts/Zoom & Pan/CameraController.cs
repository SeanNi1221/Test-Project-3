...
# else
    ...
    internal void Zoom(float delta) {
        ...
        _camera.fieldOfView = Mathf.Clamp(_camera.fieldOfView - delta, 1, _origin.Fov);

        //The scale of the view changes with fov, therefore a compensation
        //lens shift is needed to ensure that the view center stays unmoved.
        float previousScale = _canvas.Scale;
        _canvas.UpdateScale();
        float compensation = _canvas.Scale/previousScale;

        _lensShiftX = Mathf.Clamp(_lensShiftX * compensation, _canvas.XMin, _canvas.XMax);
        _lensShiftY = Mathf.Clamp(_lensShiftY * compensation, _canvas.YMin, _canvas.YMax);

        _isDirty = true;
    }
    internal void Pan(Vector2 delta) {
        _lensShiftX = Mathf.Clamp(_lensShiftX - delta.x, _canvas.XMin, _canvas.XMax);
        _lensShiftY = Mathf.Clamp(_lensShiftY - delta.y, _canvas.YMin, _canvas.YMax);

        _isDirty = true;
    }
    private void GetInputs() {
        var zoomInput = Input.GetAxis("Mouse ScrollWheel");
        _zoomDelta = zoomInput ==
            0 ? 0 : zoomInput * ZoomSpeed * InputProcessor.ScrollWheel;
